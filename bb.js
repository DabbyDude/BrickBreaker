  ...
  for(var eachRow = 0; eachRow < BRICK_ROWS; eachRow++) {
    for(var eachCol = 0; eachCol < BRICK_COLS; eachCol++) {
      ...
      if(brickGrid[arrayIndex]) {
        rect((BRICK_WIDTH*eachCol), BRICK_HEIGHT*eachRow, BRICK_WIDTH-BRICK_GAP, BRICK_HEIGHT-BRICK_GAP, 'purple');
...
function drawAll() {
  ...
  if(showEndingScreen) {
    if(playerScore == maximumScore) {
      ...
      text("Click to play again", canvas.width/2, 550, 'white', 'bold 1.5em Arial', 'center');
    } else {
  ...
  //ball
  circle(ballX, ballY, ballSize, 'white');

  //paddle
  rect(paddleX, canvas.height-PADDLE_DIST_FROM_EDGE, PADDLE_WIDTH, PADDLE_HEIGHT, 'blue');
